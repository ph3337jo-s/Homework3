...
cat /tmp/$FTTEMPDIR/fortunes |json_pp > /tmp/$FTTEMPDIR/fortunes_pp
...
if [[ "${PRINTINFO}x" == "--printinfox" ]]; then
	...
	rm -r  /tmp/$FTTEMPDIR #So it doesnt create files and leave them
 	exit 0

elif [[ "${CLEANUP}x" == "--cleanupx" ]]; then
...
fgrep message fortunes_pp > /tmp/$FTTEMPDIR/fortunes_messages
...
NUMMSG=`wc -l fortunes_messages|cut -d' ' -f1`
...
MESSAGE=`head -n+$CHOSEN /tmp/$FTTEMPDIR/fortunes_messages|tail -n1`

# E11 (1 point) extract only the message content using cut

MESSAGE=`echo -e $MESSAGE|cut -d',' -f1|cut -d':' -f2|cut -d'"' -f2`
...
rm -r /tmp/$FTTEMPDIR #didnt work with cleanup function, presumably because of "rm-ir" had to ask permission
